    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const examData = await eRes.json();

                if (!eRes.ok) throw new Error(examData.error || 'Failed to load exam');

                setQuestions(questionData);
                setExamName(examData.name);

                // Map levels from API structure to form structure
                if (examData.levels && examData.levels.length > 0) {
                    const sortedLevels = [...examData.levels].sort((a, b) => a.levelNumber - b.levelNumber);
                    setLevels(sortedLevels.map((l: any) => ({
                ...
                alert(err.message);
                router.push('/organizer/dashboard/exams');
            }
        };
    ...
    return (
        <div className="max-w-6xl mx-auto pb-20">
            <div className="flex items-center justify-between mb-10">
                <div>
                    <h1 className="text-3xl font-black italic tracking-tighter text-gray-950 uppercase">Update RACE Configuration</h1>
                    <p className="text-gray-400 font-bold italic text-sm mt-1 uppercase tracking-widest">Modifying event identity #EXT-{id.substring(0, 4).toUpperCase()}</p>
                </div>
                            ...
                            >
                                <div className="flex items-start justify-between mb-8 relative z-10">
                                    <div className="flex items-center gap-4">
                                        <div className="h-14 w-14 bg-gray-950 text-white rounded-2xl flex items-center justify-center font-black italic text-2xl">
                                            L{level.levelNumber}
                                        </div>
                                        <div>
                                ...
                                <div className="grid grid-cols-2 gap-6 mb-8 relative z-10">
                                    ...
                                    <div className="space-y-2 col-span-2">
                                        <label className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] italic ml-1">
                                            <Clock size={12} /> Opening Schedule (Optional)
                                        </label>
                                        <input
                                            type="datetime-local"
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {level.questionIds.length > 0 ? (
                                            level.questionIds.map(qid => (
                                                <div key={qid} className="bg-blue-50 text-blue-600 px-4 py-2 rounded-xl text-xs font-black italic flex items-center gap-2 border border-blue-100 group">
                                                    {questions.find(q => q.id === qid)?.title}
                                                    <button onClick={() => toggleQuestionInLevel(idx, qid)} className="text-blue-300 hover:text-blue-600">
                                                        <X size={14} />
                                                    </button>
                                                </div>
                                            ))
